...
- 支持“聚合搜索”，一次并行搜索多个普通图片与动画站点，自动合并、去重并保留来源信息；AI 和福瑞站点不会加入聚合搜索。
- 支持图片与视频分类、标签搜索、内容分级、日期周期和尺寸筛选。
- 支持等高网格与瀑布流、图片懒加载、滚动加载和响应式布局。
- 支持图片缩放、视频自动播放、静音、循环和播放进度保存。
- 支持收藏、收藏标签与备注、浏览历史、稍后观看播放列表、搜索集和智能收藏夹。
- 支持单项下载、选择后批量下载、下载当前收藏夹及下载全部收藏。
- 支持明亮/深色主题、四种主题色、Material 状态层与按压反馈、紧凑网格、敏感内容模糊和减少动画。
...
- 点击媒体卡片或预览窗口的书签按钮，可加入“稍后看”页；视频播放进度会沿用现有本地进度记录。
...
- “导出收藏”生成格式化 JSON，内容包括收藏、历史、稍后观看、搜索集和智能收藏夹。
- “导入收藏”会合并收藏、历史和稍后观看，并恢复导出文件中的搜索集和智能收藏夹。
...
将唯一的脚本下载或复制到 Ubuntu/Debian 服务器后直接执行；脚本会自动判断首次部署或更新：

```bash
curl -fsSL https://raw.githubusercontent.com/Neptune326/codex-anima-html/main/deploy/atlas-gallery.sh -o atlas-gallery.sh && sudo bash atlas-gallery.sh
```

每个步骤均显示中文开始和完成状态。更新模式会强制同步 `origin/main` 并清理部署目录中的本地修改与未跟踪文件，服务器环境配置保存在仓库外且不会被覆盖。
